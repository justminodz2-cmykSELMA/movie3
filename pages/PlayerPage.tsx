import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react';import { useLocation, useNavigate } from 'react-router-dom';import VideoPlayer from '../components/Player';import { Movie, Episode, Season, HistoryItem } from '../types';import { fetchFromTMDB } from '../services/apiService';import { useProfile } from '../contexts/ProfileContext';import { useTranslation } from '../contexts/LanguageContext';import { usePlayer, PipData } from '../contexts/PlayerContext';import { IMAGE_BASE_URL, BACKDROP_SIZE_MEDIUM } from '../contexts/constants';import { GoogleGenAI, Type } from "@google/genai";

interface ScheduledItem {title: string;duration: number; // in minutesreason: string;type: 'movie' | 'tv';}

interface Schedule {items: ScheduledItem[];}

const scheduleSchema = {type: Type.OBJECT,properties: {items: {type: Type.ARRAY,description: "A list of scheduled items for the day.",items: {type: Type.OBJECT,properties: {title: {type: Type.STRING,description: "The exact title of the movie or TV show.",},duration: {type: Type.NUMBER,description: "The duration in minutes for this block. For a TV show, this is the total time for multiple episodes. For a movie, it's just the movie's length.",},reason: {type: Type.STRING,description: "A fun, kid-friendly reason why this show is being aired now. e.g., 'Time for some adventure!'",},type: {type: Type.STRING,description: "The type of content. Must be either 'movie' or 'tv'.",},},required: ["title", "duration", "reason", "type"],},},},required: ["items"],};

const PlayerPage: React.FC = () => {const location = useLocation();const navigate = useNavigate();const { item: initialItem, type, season: initialSeason, episode: initialEpisode, currentTime, streamUrl, liveChannels, currentChannelIndex, logo, needsProxy } = location.state || {};const { setToast, updateHistory, getScreenSpecificData, isKidsMode } = useProfile();const { t } = useTranslation();const { setPipData, setPipAnchor } = usePlayer();
